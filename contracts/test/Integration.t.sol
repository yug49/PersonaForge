...
contract IntegrationTest is Test {
    ...
    function test_RoleBasedWorkflow() public {
        ...
        vm.startPrank(updater1);
        vm.expectRevert("Not authorized to update data");
    ...
    function test_TransferWorkflowIntegration() public {
        ...
        string memory response1 = personaINFT.interactWithAgent(tokenId, "What's the best strategy for chess?");
        assertTrue(bytes(response1).length > 0);
        ...
        string memory response2 = personaINFT.interactWithAgent(tokenId, "What about poker strategy?");
        assertTrue(bytes(response2).length > 0);
    ...
    function test_PerformanceUnderLoad() public {
